...
从零构建一个**生产级全栈 AI Agent**，整合 6 大核心能力模块，实现智能路由、多轮记忆、知识增强检索、工具编排与多步推理。后端使用 Go，前端单页应用，支持真实 LLM API 对接与完整基础设施集成。
...
final/
├── config/
│   ├── config.go          # 配置加载（yaml.v3）
│   └── config.yaml        # 唯一配置源
├── internal/
│   ├── agent/
│   │   └── agent.go       # 核心调度器：路由/ReAct/Planner/Generator/Memory
│   ├── llm/
│   │   └── llm.go         # LLM Client：Chat / Embed / ExtractPreferences
│   ├── memory/
│   │   └── memory.go      # 三层记忆：STM / LTM(Embedding) / Preference
│   ├── rag/
│   │   └── rag.go         # RAG Engine：分割/向量化/检索/LLM合成
│   ├── tools/
│   │   └── tools.go       # 工具注册：内置工具 + MCP工厂
│   ├── handler/
│   │   └── handler.go     # HTTP Handler：所有 API 路由
│   └── infra/
│       └── infra.go       # 基础设施：PG/Milvus/ES/Kafka
├── frontend/
│   └── index.html         # 单页前端
└── main.go
```
